# Create a partially signed transaction from the details below
TX=""
TXID=$(bitcoin-cli -regtest decoderawtransaction "$TX" | jq -r '.txid')
bitcoin-cli -regtest createpsbt "[{\"txid\":\"$TXID\",\"vout\":0},{\"txid\":\"$TXID\",\"vout\":1}]" "[{\"2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP\":0.20000000}]"
